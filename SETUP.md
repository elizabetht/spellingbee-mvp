...
## Cluster Nodes (example from `microk8s kubectl get nodes -o wide`)
...
The manifest pins workloads using `nodeSelector`:
...
Most browsers require HTTPS for mic access on non-localhost origins.
...
To deploy only the gateway + UI first:
